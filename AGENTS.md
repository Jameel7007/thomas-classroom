...
The established Fraunces, Albert Sans, Newsreader, IBM Plex Sans, and IBM Plex
Mono files are self-hosted by Astro through `src/styles/fonts.css` and
`public/fonts/`. Do not reintroduce visitor-time Google Fonts requests. Keep the
font notices, checksums, cache policy, and `npm run fonts:validate` gate intact.
...
- pre-generated static listening clips with `data-voice-clip`
...
Listening audio is generated only during authoring and served as static MP3 files
from `astro-pilot/public/audio/`; never place an ElevenLabs API key in HTML,
browser JavaScript, public environment variables, or committed files. Approved
clip text and stable output paths live in `astro-pilot/private/voice-scripts.json`.
Keep `data-speak` as a missing-file browser-speech fallback. Production playback
must never call ElevenLabs. See `ELEVENLABS.md` for setup and authoring.
...
The Dictionary tab is now a native, curated word-clearing tool. Its validated
registry and progressive interface let a tutor search during a lesson, compare
learner-friendly senses, inspect word origins, examples, chunks, and close
words, link into relevant lessons, and mark individual meanings clear for the
current session.

Clear marks must remain temporary and must not become student progress tracking.
Expand the registry from observed lesson friction rather than padding it with
thin definitions. If lookup becomes networked later, keep API keys and provider
requests server-side and preserve a useful static collection when the service is
unavailable.

## Language Transfer Direction

The Languages tab is the home for L1-to-English transfer work. Its validated
registry currently contains 27 common and less obvious Spanish, Portuguese, and
Turkish patterns, each linked to one or two ready lessons through canonical
lesson IDs.
...
Keep each pattern framed as a hypothesis to test against actual learner output,
not a trait assigned to every speaker. A pattern must explain the source-language
logic, show a clearly labeled possible transfer and English rebuild, provide
natural examples and a tutor move, and resolve to a ready lesson. Add new
patterns in `astro-pilot/src/data/language-transfer.mjs`; do not create a second
page registry. The patterns that repeatedly need deeper practice should become
dedicated production lessons over time.
...
The build also generates `dist/_redirects` from the canonical route inventory.
Do not hand-maintain a second redirect list or add a static `public/_redirects`
file. For a public release, follow `astro-pilot/DEPLOYMENT.md`, build with the
real `SITE_URL`, and run `npm run live:validate -- https://the-public-origin`
after deployment so host-level redirects, headers, caching, audio, and direct
routes are verified rather than inferred from local output.

Before a public release, or after changing shared interaction behavior, run
`npm run qa:release`. This builds the site and exercises the generated homepage,
curriculum finder, representative lesson feedback, quick diagnostic, Dictionary,
and Language Transfer guide in an installed Chromium browser. Use
`BROWSER_QA_URL` only when deliberately testing an already running local preview.
